        ...
        with x2:
            with st.container():
                ...
                if st.button("Save Score"):
                    new_score = int(st.session_state.score)
                    today = datetime.now().strftime("%m/%d/%Y")

                    # Retrieve the top 10 scores from the database
                    top_scores = conn.execute(text("SELECT * FROM players ORDER BY Score DESC LIMIT 10")).fetchall()

                    # Check if the new score qualifies to be in the top 10
                    if len(top_scores) < 10 or new_score > top_scores[-1].Score:
                        if len(top_scores) == 10:
                            # Delete the lowest score if there are already 10 scores
                            lowest_score_id = top_scores[-1].uuid
                            conn.execute(text("DELETE FROM players WHERE uuid = :uuid"), {'uuid': lowest_score_id})

                        # Insert the new score
                        conn.execute(text('''
                                INSERT INTO players (uuid, Date, Name, Score) 
                                VALUES (:uuid, :Date, :Name, :Score);
                            '''), {
                                'uuid': str(uuid.uuid4()),
                                'Date': today,
                                'Name': name, 
